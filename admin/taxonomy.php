<?php
function auiu_taxonomy_fields() {
    $action = ( isset( $_GET['action'] ) ) ? $_GET['action'] : '';
    switch ($action) {
        case 'edit':
            auiu_taxonomy_fields_edit();
            break;
        default:
            auiu_taxonomy_fields_main();
            break;
    }
}
function auiu_taxonomy_fields_main() {
    ...
    <div class="wrap auiu-admin">
        ...
        <form action="" method="post" style="margin-top: 20px;">
            ...
            <table class="widefat meta" style="width: 850px">
                ...
                <?php auiu_build_form( $taxonomy_fields ); ?>
            ...
            <input name="auiu_add_custom" type="submit" class="button-primary" value="<?php _e( 'Add Field' ) ?>" style="margin-top: 10px;" />
        </form>
        ...
        <table class="widefat meta" style="margin-bottom: 20px;">
            ...
            if ( $wpdb->num_rows > 0 ):
                ...
                foreach ($fields as $row):
                    //var_dump( $row );
                    if ( !auiu_starts_with( $row->field, 'ct_' ) ) {
                        continue;
                    }
                    ?>
                    <tr valign="top" <?php echo ( ($count % 2) == 0) ? 'class="alternate"' : ''; ?>>
                        ...
                        <td>
                            <a href="admin.php?page=auiu_custom_tax&action=edit&id=<?php echo $row->id; ?>"><img src="<?php echo plugins_url( 'wp-user-frontend/images/edit.png' ); ?>"</a>
                            <a href="<?php echo wp_nonce_url( "admin.php?page=auiu_custom_tax&action=del&id=" . $row->id, 'auiu_del' ) ?>" onclick="return confirm('Are you sure to delete this field?');"><img src="<?php echo plugins_url( 'wp-user-frontend/images/del.png' ); ?>"</a>
                        </td>
                    </tr>
                    <?php $count++;
                endforeach; ?>
        ...
        <?php if ( $row ): ?>
            <form action="" method="post" style="margin-top: 20px;">
                ...
                <table class="widefat meta" style="width: 850px">
                    ...
                    <?php auiu_build_form( $custom_fields, $values, false ); ?>
                    <tr valign="top" id="auiu_field_values_row" style="display: none;">
                        <td scope="row" class="label"><label for="auiu_field_values">Values</label></td>
                        <td>
                            <textarea name="field_values" id="auiu_field_values" cols="30"><?php echo $row->values; ?></textarea>
                            <span class="description"><br>This will be used as option fields. Please separate values with comma</span>
                        </td>
                    </tr>
                </table>
                <input name="auiu_edit_custom" type="submit" class="button-primary" value="<?php _e( 'Update Field' ) ?>" style="margin-top: 10px;" />
            </form>
        <?php else: ?>
            <h2>Nothing found</h2>
        <?php endif; ?>
